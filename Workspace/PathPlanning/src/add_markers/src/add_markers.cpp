...
// Define Odometry callback function
void odometry_cb(const nav_msgs::Odometry::ConstPtr& msg) 
{
	...
	if(!pick_up && !drop_off)
	{
		...
		ROS_INFO("Distance to Location = %f", pickup_dis);
		
		if(pickup_dis <= tolerance)
		{
			ROS_INFO("Arrived At Location");
			pick_up = true;
		}
	}
	if(pick_up && !drop_off)
	{
		...
		ROS_INFO("Distance To Destination = %f", dropoff_dis);
	...
	marker.lifetime = ros::Duration();
  	...
  	while (ros::ok())
  	{
		...
		if(pick_up)
		{
			...
			ROS_INFO("Object Picked Up From Location!");
		...
		if(drop_off)
		{
			...
			marker.action = visualization_msgs::Marker::ADD;
			ROS_INFO("Object Dropped At Destination!");
			ros::Duration(2.0).sleep();
		}
